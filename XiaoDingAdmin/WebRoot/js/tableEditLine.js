$(function(){
	...
	initFunc(c.order, rowsTd); 
	  ...
	  */
	  function getEditableCols() { 
		   ...
		   if($.type(c.editableCols) != "array" && cols == "all") 
		   { //如果是所有列都可以编辑的话
			    cols = []; 
		   ...
		   else if($.type(c.editableCols) == "array")
		   { //有指定选择编辑的列的话需要排序放置“编辑”功能的列
		    var copyCols = []; 
	 ...
	 /** 
	  ...
	  function createEdit(operateCol) 
	  {
		   $(editLink).appendTo(operateCol).on("click", function() { 
			    if(replaceQuote($(this).html()) == replaceQuote(c.edit)) 
			    { //如果此时是编辑状态
			     toSave(this); //将编辑状态变为保存状态 
			    } 
			    else if(replaceQuote($(this).html()) == replaceQuote(c.save)) 
			    { //如果此时是保存状态
				     ...
				     c.saveCallback($(this).closest('tr'), data, function() { 
				      toEdit($this, true); 
				     }); 
			    } //end else if
		   }); 
		   ...
		   } //end function toEdit(ele, isSave)
	  } // end function createEdit(operateCol) 
	  /** 
	   * 创建“添加一行”的功能 
	   * @param operateCol 
	   */ 
	  ...
	  } //end  function createAdd(operateCol)
	  ...
	  function createDel(operateCol)
	  {
		   $(delLink).appendTo(operateCol).on("click", function() { 
			    //var _this = this;
			    var curLine = $(this).closest('tr');
			    c.delCallback(curLine, function() {
			        $(curLine).remove(); 
			    }); 
		   }); 
	  } 
  ...
  };//end $.fn.handleTable = function (options)
